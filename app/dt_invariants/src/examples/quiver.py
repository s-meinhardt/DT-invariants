...
class Quiver:
    def __init__(self, num_vertices: int, arrow_matrix: dict[tuple[int, int], int], name: Optional[str] = None) -> None:
        ...
        self.hom: ScalarProduct = ScalarProduct(rank=num_vertices)  # the standard scalar product
        self.ext: ScalarProduct = ScalarProduct(rank=num_vertices, coeff_matrix=arrow_matrix)
        ...
        self.euler_pairing: ScalarProduct = ScalarProduct(
...
# euler_pairing = ScalarProduct(rank=quiver.num_vertices, coeff_matrix={**matrix, **matrix_tr})
